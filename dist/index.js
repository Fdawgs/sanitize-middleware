...
function parseValue(value, type) {
    ...
    switch (type.toLowerCase()) {
        ...
        default:
            ...
            result = validator.stripLow(xss_1.filterXSS(sanitize(value))).trim();
            break;
    }
    return result;
}
...
function parseValues(args, config) {
    ...
    Object.keys(config).forEach((configKey) => {
        if (config[configKey].mandatory &&
            config[configKey].mandatory === true) {
            mandatoryArgs.push(configKey);
        }
    });
    if (mandatoryArgs.every((element) => keys.map((x) => x.toLowerCase()).includes(element.toLowerCase())) === false) {
